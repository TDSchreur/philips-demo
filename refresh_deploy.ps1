...
helm upgrade --install philips-demo `
    -n localdev `
    --set ingress.path="/philips-api-local" `
    --set image.repository="host.docker.internal:5000/philips-api-localbuild" `
    --set image.tag=$tag .chart